...
/*	@Transactional(value="bank_xyz_txm_qual",propagation=Propagation.REQUIRED,isolation=Isolation.READ_COMMITTED,timeout=5)
	private void selectFromBank_xyz() throws Exception{
		System.out.println(" :: From bank bank_xyz :: ");
		try (Connection connection = this.getBank_xyz_ds().getConnection();
				Statement statement = connection.createStatement();
				ResultSet rs = statement.executeQuery(SELECT_SQL)) {
			while(rs.next()){
				System.out.println(" :: ACC_NUMBER :: " + rs.getInt(ACC_NUMBER));
				System.out.println(" :: CUSTOMER_NAME :: " + rs.getString(CUSTOMER_NAME));
				System.out.println(" :: ACC_BALANCE :: " + rs.getInt(ACC_BALANCE));
			}
	...
	private void selectFromBank_abc() throws Exception{
		System.out.println(" :: From bank bank_abc :: ");
		try (Connection connection = this.getBank_abc_ds().getConnection();
				Statement statement = connection.createStatement();
				ResultSet rs = statement.executeQuery(SELECT_SQL)) {
			while(rs.next()){
				System.out.println(" :: ACC_NUMBER :: " + rs.getInt(ACC_NUMBER));
